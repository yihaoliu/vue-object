  ...
  <div class="j-content">
      ...
      <transition-group name="fade" tag="div">
        <div v-for="(photo,index) in photoLists" :key="photo" class="item" @click="getPhotoDetail(index)">
            <img class="animated bounceIn" :src="photo.src">
        </div>
      </transition-group>
    </div>
...
export default {
  ...
  methods: {
    getPhotoDetail(index){
      ...
      this.$store.commit("PHOTO_DETAIL",photoData)
    ...
    loadMore: function() {
        ...
        return axios.get('./data/pictures.json')
        .then(function(res) {
          /*var arr = []
          ...
          res.data.map(function(item,index){
            item._id = index;
            _this.photoLists.push(item);
          })
          /*res.data.results.forEach(photo=>{
              console.log(photo,"kkkkk")
              this.photoLists.push(photo);
          })*/
          this.busy = false;
