...
<div id="movielist-top250" class="j-container">
  <div class="j-content">
    <md-list class="custom-list md-triple-line">
        <md-list-item v-for="movie in movieLists" @click="getMovieDetail(movie)">
            <md-avatar>
              <img :src="movie.src" alt="People">
            </md-avatar>

            <div class="md-list-text-container">
              <span>{{movie.title}}</span>
              <p>({{movie.time}})(作者：{{movie.author}})</p>
            </div>
...
export default {
  ...
  methods: {
    getMovieDetail(data){
      ...
      this.$router.push({ name: 'movie-detail', params: { id: data.id }})
      this.$store.commit("DETAIL_DATA",data)
    },
	...
	loadMore: function() {
      ...
      axios.get('./data/ranking.json')
		  .then(function(res) {
        let items = Object.assign(res.data);
		    items.map(function(item,index){
          item.id = index;
          _this.movieLists.push(item);
        })
